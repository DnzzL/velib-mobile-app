...
class Api {
  ...
  Future<StationStatus> fetchNumberBikesAtStation(int stationId) async {
    var url = '$root/state/$stationId';
    var response = await http.get(url);
    if (response.statusCode == 200) {
      StationStatus state =
          StationStatus.fromJson(jsonDecode(utf8.decode(response.bodyBytes)));
      return state;
    } else {
  ...
  Future<List<Station>> fetchClosestStations(
